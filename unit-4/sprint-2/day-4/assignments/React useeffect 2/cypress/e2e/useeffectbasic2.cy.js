...
data.forEach(({ submission_link: url, id }) => {
  describe("useEffect assignement 2", function () {
    let acc_score = 1;
    ...
    it("Check if initial network request is made and appended 10 results to tbody", () => {
    ...
    it("check it default page is 1 and button is disabled and page 1 results are correct", () => {
      ...
      cy.get("tbody").children().should("have.length", 10);
      checktablecontents(apidata,0,9)     
      cy.then(() => {
        acc_score += 2;
      });
    });
    ...
    it("The border color of the active button should be red",()=>{
      ...
      cy.visit(url);      
      cy.wait("@initialrequest");
      for(let page=1;page<9;page=page+3)
      {        
        cy.get("button").eq(page).click({force: true});
        cy.get("button").eq(page).should("be.disabled");
        cy.wait("@initialrequest");       
    ...
    it("check pagination component is dynamic",()=>{
      cy.intercept("GET",`**mockapi/get-employees**`,{data:[{...apidata,...apidata}],totalPages:20}).as("reqdata");
      cy.visit(url);
      cy.wait("@reqdata")
